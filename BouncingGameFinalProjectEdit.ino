...
void generateMIDI() //formula for creating a midi
{
  for (int x = 0; x < 127; ++x)
  {
...
void setup() { //Sets up interrupts, serial monitor, and initial serial monitor print
...
void loop() {
  ...
  if (i == 1 || i == 8){ //game end state, reads for light to be past a specific threshold and resets back to base values
    ...
    rbuttonflag = false; //Resets flags so that there are no backlogged inputs and the game doesn't start by itself
    lbuttonflag = false;
    i=2;
    speedvalue = 500;
    score = 0;
